...
parser.add_argument(
    ...
    help="Target object from {bottle, car, dorrstop} (default bottle)"
)
parser.add_argument(
    '--render_num', type=int, default=10,
...
class NeuromekaRenderDB():
    def __init__(self, cls_type, render_num=10, rewrite=False):
        ...
        self.neuromeka_dir = '../ffb6d/datasets/neuromeka'
        self.render_dir = os.path.join(self.neuromeka_dir, 'renders', cls_type)
        check_dir(self.render_dir)
        self.render_num = render_num
        RT_pth = os.path.join(self.neuromeka_dir, 'sampled_poses', '{}_sampled_RTs.pkl'.format(cls_type))
        self.RT_lst = pkl.load(open(RT_pth, 'rb'))

        so_p = './rastertriangle_so.so'
        self.dll = np.ctypeslib.load_library(so_p, '.')

        self.bg_img_pth_lst = glob("../ffb6d/datasets/SUN2012pascalformat/JPEGImages/*.jpg")
        ...
        b_mdl_p = os.path.join(
            self.neuromeka_dir, 'models', '{}_obj_res.ply'.format(cls_type)
        )
...
def main():
    ...
    for cls_type in cls_list:
        ...
        print("cls: ", cls_type)
        gen = NeuromekaRenderDB(cls_type, render_num, True)
