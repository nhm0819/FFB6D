...
from datasets.neuromeka.preprocs import read_objects, read_view, calc_Tco,\
    scaleback_depthmap, rescale_depthmap, combine_bytes_arr
...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="bottle", batch_size=0, DEBUG=False):
        ...
        print("cls_id in neuromeka_dataset.py", self.cls_id)
        self.root = self.config.neuromeka_root
        self.cls_root = os.path.join(self.root, self.cls_type)
        self.rng = np.random
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        ...
        cls_type, folder_name, file_name = real_item.split("/")

        # depth
        with Image.open(os.path.join(self.cls_root, folder_name, "depthmaps", file_name + ".png")) as di:
            dpt_buff = np.array(di)
        real_dpt = combine_bytes_arr(dpt_buff[:, :, 1], dpt_buff[:, :, 2])

        # mask
        with Image.open(os.path.join(self.cls_root, folder_name, "masks", file_name + f"_00_{self.cls_id:02d}.png")) as li:
        ...
        # rgb
        with Image.open(os.path.join(self.cls_root, folder_name, file_name + ".png")) as ri:
    ...
    def get_item(self, item_name):
        ...
        if split_name[-2] != self.cls_type:
            cls_type, folder_name, file_name = item_name.split("/")
        ...
        if "pkl" in item_name:
            ...
            with Image.open(os.path.join(self.cls_root, folder_name, "depthmaps", file_name+".png")) as di:
                dpt_buff = np.array(di)
            # dpt_mm = scaleback_depthmap(combine_bytes_arr(dpt_buff[:, :, 1], dpt_buff[:, :, 2]) / DEPTH_SCALE_HQ)
            dpt_mm = combine_bytes_arr(dpt_buff[:, :, 1], dpt_buff[:, :, 2])
            with Image.open(os.path.join(self.cls_root, folder_name, "masks", file_name+f"_00_{self.cls_id:02d}.png")) as li:
                labels = np.array(li)
                labels = (labels > 0).astype("uint8")
            with Image.open(os.path.join(self.cls_root, folder_name, file_name+".png")) as ri:
    ...
    def get_pose_gt_info(self, cld, labels, RT):
        ...
        for i, cls_id in enumerate([1]):
            ...
            ctr = self.bs_utils.get_ctr(self.cls_type, ds_type="neuromeka")[:, None]
