...
from preprocs import read_objects, read_view, calc_Tco

# config = Config(ds_name='neuromeka')
...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck", DEBUG=False):
        self.DEBUG = DEBUG
        self.config = Config(ds_name='neuromeka')
        ...
        self.cls_lst = self.bs_utils.read_lines(self.config.neuromeka_cls_lst_p)
        ...
        if dataset_name == 'train':
            ...
            tst_img_pth = os.path.join(self.root, "test_data_list.txt")
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        ...
        folder_name, file_name = real_item.split("/")
        cls_id = os.listdir(os.path.join(self.root, folder_name, "stl"))[0][2]

        with Image.open(os.path.join(self.root, "depthmaps", real_item+'.png')) as di:
            real_dpt = np.array(di)
        with Image.open(os.path.join(self.root, "masks", f'{real_item}_00_{cls_id:02d}.png')) as li:
            bk_label = np.array(li)
        bk_label = (bk_label < 255).astype(rgb.dtype)
        if len(bk_label.shape) > 2:
            bk_label = bk_label[:, :, 0]
        with Image.open(os.path.join(self.root, real_item+'.png')) as ri:
    ...
    def get_item(self, item_name):
        folder_name, file_name = item_name.split("/")
        cls_id = int(os.listdir(os.path.join(self.root, folder_name, "stl"))[0][:2])
        ...
        if "pkl" in item_name:
            ...
            with Image.open(os.path.join(self.root, "depthmaps", file_name+'.png')) as di:
                dpt_mm = np.array(di)
            with Image.open(os.path.join(self.root, "masks", file_name+f'_00_{cls_id:02d}.png')) as li:
                labels = np.array(li)
                labels = (labels > 0).astype("uint8")
            with Image.open(os.path.join(self.root, item_name)) as ri:
                if self.add_noise:
                    ri = self.trancolor(ri)
                rgb = np.array(ri)[:, :, :3]

            view = os.path.join(os.path.join(self.root, folder_name, "poses", file_name + ".csv"))
            obj = os.path.join(os.path.join(self.root, folder_name, "config", file_name + ".csv"))
            RT = np.matmul(self.Tx180, calc_Tco(obj, view))[:3]
            rnd_typ = 'real'
        ...
        RTs, kp3ds, ctr3ds, cls_ids, kp_targ_ofst, ctr_targ_ofst = self.get_pose_gt_info(
            cld, labels_pt, RT, cls_id=cls_id
        )
    ...
    def get_pose_gt_info(self, cld, labels, RT, cls_id=1):
        ...
        for i, _ in enumerate([1]):
            ...
            ctr = self.bs_utils.get_ctr(cls_id, ds_type="neuromeka")[:, None]
            ...
            kps = self.bs_utils.get_kps(
                self.cls_type, kp_type=kp_type, ds_type='neuromeka'
            )
