...
class Trainer(object):
    ...
    def eval_epoch(self, d_loader, is_test=False, test_pose=False, it=0):
        ...
        if args.local_rank == 0:
            wandb.log({'val_acc': acc_dict['acc']})
...
parser.add_argument('--train_bs', type=int, default=8, metavar='N',
                    help='train batch size / num_gpus')
parser.add_argument('--val_bs', type=int, default=8, metavar='N',
                    help='validation batch size / num_gpus')
parser.add_argument('--test_bs', type=int, default=8, metavar='N',
                    help='test batch size / num_gpus')
...
parser.add_argument('--wandb_project', type=str, default="neuromeka")
...
def main():
    ...
    wandb_logger = WandbLogger(project=args.wandb_project, name=f"{args.cls}-{args.wandb_name}", log_model=False)
    ...
    ealry_stop_callback = EarlyStopping(monitor='val_loss', patience=20, verbose=False, mode='min')
    checkpoint_callback = ModelCheckpoint(monitor='val_loss', dirpath=MODEL_CKPT_PATH, filename=MODEL_CKPT, save_top_k=10, mode='min')

    # setup data
    neuromeka_data = NeuromekaDataModule(args=args, batch_size=args.batch_size, num_workers=args.num_workers)

    # setup model
    model = pl_ffb6d(args=args, lr=args.lr, pretrained=True)
    ...
    trainer = pl.Trainer(
        ...
        gpus=-1,                # use all GPUs
        max_epochs=config.n_total_epoch // args.gpus, # number of epochs
